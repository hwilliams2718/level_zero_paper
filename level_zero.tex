...
\newcommand{\scw}{\vec{\operatorname{s}}}
\newcommand{\sccw}{\cev{\operatorname{s}}}
\newcommand{\ncw}{\vec{\operatorname{n}}}
\newcommand{\nccw}{\cev{\operatorname{n}}}
...
\begin{proposition}
  ...
  \begin{equation}
    ...
    =
    \begin{cases}
      c^{\left\lfloor\frac{m}{n}\right\rfloor}s_1\cdots s_{\mod{m}} \alpha_i & \text{if $m>0$}\\
      \alpha_i & \text{if $-n\leq m \leq 0$ and  $i\leq n+m$} \\
      -\alpha_i & \text{if $-n\leq m \leq 0$ and $i>n+m$} \\
      -c^{2+\left\lfloor\frac{m+1}{n}\right\rfloor}s_n\cdots s_{\mod{m+1}} \alpha_i & \text{if $m <-n$}\\
    \end{cases}
  \end{equation}
  Where $\left\lfloor \frac{m}{n} \right\rfloor$ is the integer part of $\frac{m}{n}$.
  Moreover, whenever $m > 0 $ or $m < -n$, the vector $\gamma_{m,\mod{m}}$ is negative and the vector $\gamma_{m,\mod{m+1}}$ is positive.
\end{proposition}

\begin{remark}
  If $m>0$ then $c^{\left\lfloor\frac{m}{n}\right\rfloor}s_1\cdots s_{\mod{m}}$ is the prefix of $c^\infty$ of length $m$.
  Similarly, if $m<-n$, $c^{2+\left\lfloor\frac{m+1}{n}\right\rfloor}s_n\cdots s_{\mod{m+1}}$ is the prefix of $c^{-\infty}$ of length $-m-n$.
\end{remark}
...
\begin{proof}
  ...
  When $m>0$ the root $c^{\left\lfloor\frac{m}{n}\right\rfloor}s_1\cdots s_{\mod{m}} \alpha_{\mod{m+1}}$ is positive: being a prefix of $c^\infty$, the word  $c^{\left\lfloor\frac{m}{n}\right\rfloor}s_1\cdots s_{\mod{m}} s_{\mod{m+1}}$ is reduced (cf. \cite{Spe09}) and we can apply \cite[VI \S 1.6 Corollary 2 to Proposition 17]{Bou02}.
  By the same principle the root $c^{\left\lfloor\frac{m}{n}\right\rfloor}s_1\cdots s_{\mod{m}} \alpha_{\mod{m}} = -c^{\left\lfloor\frac{m}{n}\right\rfloor}s_1\cdots s_{\mod{m-1}} \alpha_{\mod{m}}$ is negative because the word $c^{\left\lfloor\frac{m}{n}\right\rfloor}s_1\cdots s_{\mod{m}}$ is reduced.
  Similarly, when $m<-n$ the root $-c^{2+\left\lfloor\frac{m+1}{n}\right\rfloor}s_n\cdots s_{\mod{m+1}} \alpha_{\mod{m+1}}$ is positive and the root $-c^{2+\left\lfloor\frac{m+1}{n}\right\rfloor}s_n\cdots s_{\mod{m+1}} \alpha_{\mod{m}}$ is negative.
  ...
  \begin{align*}
    ...
    s_{\gamma_{m,\mod{m+1}}}\big(\gamma_{m,i}\big)\\
    &=
    \big(c^{\left\lfloor\frac{m}{n}\right\rfloor}s_1\cdots s_{\mod{m}}\big)s_{\mod{m+1}}\big(s_{\mod{m}}\cdots s_1 c^{-\left\lfloor\frac{m}{n}\right\rfloor}\big)\big(c^{\left\lfloor\frac{m}{n}\right\rfloor}s_1\cdots s_{\mod{m}}\alpha_i\big)\\
    &= 
    c^{\left\lfloor\frac{m}{n}\right\rfloor}s_1\cdots s_{\mod{m}}s_{\mod{m+1}} \alpha_i.
  \end{align*}
...
\begin{proposition}
  ...
  \begin{equation}
    ...
    =
    \begin{cases}
      c^{\left\lfloor\frac{m}{n}\right\rfloor}s_1\cdots s_{\mod{m}} \omega_i & \text{if $m>0$}\\
      \omega_i & \text{if $-n\leq m \leq 0$ and  $i\leq n+m$} \\
      -\omega_i & \text{if $-n\leq m \leq 0$ and $i>n+m$} \\
      -c^{2+\left\lfloor\frac{m+1}{n}\right\rfloor}s_n\cdots s_{\mod{m+1}} \omega_i & \text{if $m <-n$}\\
    \end{cases}
  \end{equation}
\end{proposition}
...
\begin{equation}
  ...
  =
  \begin{cases}
    c^{\left\lfloor\frac{m}{n}\right\rfloor} \omega_i & \text{if $m>0$ and $i > \mod{m}$}\\
    c^{1+ \left\lfloor\frac{m}{n}\right\rfloor} \omega_i & \text{if $m>0$ and $i \leq \mod{m}$}\\
    \omega_i & \text{if $-n\leq m \leq 0$ and  $i\leq n+m$} \\
    -\omega_i & \text{if $-n\leq m \leq 0$ and $i>n+m$} \\
    -c^{2+\left\lfloor\frac{m+1}{n}\right\rfloor} \omega_i & \text{if $m <-n$ and $i < \mod{m+1}$}\\
    -c^{1+\left\lfloor\frac{m+1}{n}\right\rfloor} \omega_i & \text{if $m <-n$ and $i \geq \mod{m+1}$}\\
  \end{cases}
\end{equation}
...
\section{Appendix: Quiver Representations}
  \subsection{Notation to recall}
  \begin{itemize}
    \item
      For $a$ arrow, \emph{tail} $t(a)$ and \emph{head} $h(a)$
  \end{itemize}

\subsection{Affine Tubes}
Let $Q$ be an acyclic affine quiver of type $A_n^{(1)}$, in particular $Q$ contains at least one source.  
  Suppose $Q$ has $p$ clockwise pointing arrows $\{\vec{a}_1,\ldots,\vec{a}_p\}$ labeled counterclockwise around $Q$ and $q$ counterclockwise pointing arrows $\{\cev{a}_1,\ldots,\cev{a}_q\}$ labeled clockwise around $Q$ so that $t(\vec{a}_p)=t(\cev{a}_q)$. 
  \sayDR{This seemingly strange choice of labeling will make extensions go in the right directions later and thus keep the arrows in the AR quiver pointing from left to right.  The phrase ``against the flow'' might be appropriate to include somewhere.}  
  It will be convenient to allow arbitrary integer labeling of the arrows taken modulo $p$ and $q$ respectively.  
  Thus $Q$ takes the following form:
  \saySS{The quiver is vertical because it generates slightly less confusion with clockwise and counterclockwise.}

  \begin{center}
    \begin{tikzpicture}[node distance=1cm]
      \usetikzlibrary{positioning}
      \tikzstyle{vertex}=[draw,shape=circle, inner sep=1.2pt, fill=black]
      \tikzstyle{arrow}=[line width=0.4pt, ->, >=stealth, shorten <=2pt, shorten >=2pt, label distance=1pt]
      \tikzstyle{omissis}=[line width=0.6pt, style=dotted, shorten <=2pt, shorten >=2pt]

      \node[vertex, label=above:$\scriptstyle \scw(h(\vec{a}_1)){=\,}\sccw(h(\cev{a}_1))$] (v0) at (0,0) {};
      \node[vertex, below left = of v0] (v1) {};
      \node[vertex, below = of v1, label=left:$\scriptstyle \ncw(h(\vec{a}_1))$] (v2) {};
      \node[vertex, below = of v2, label=left:$\scriptstyle h(\vec{a}_1)$] (v3) {};
      \node[vertex, below = of v3, label=left:$\scriptstyle \nccw(h(\vec{a}_1)){\,=\,}t(\vec{a}_1)$] (v4) {};
      \node[vertex, below right = of v0] (v8) {};
      \node[vertex, below = of v8, label=right:$\scriptstyle \nccw(h(\cev{a}_1))$] (v7) {};
      \node[vertex, below = of v7, label=right:$\scriptstyle h(\cev{a}_1)$] (v6) {};
      \node[vertex, below = of v6, label=right:$\scriptstyle t(\cev{a}_1){\,=\,}\ncw(h(\cev{a}_1))$] (v5) {};

      \draw[arrow] (v0) -- node[above left=-3pt ] {$\scriptstyle \cev{a}_q$} (v1);
      \draw[omissis] (v1) -- (v2);
      \draw[arrow] (v2) -- (v3);
      \draw[arrow] (v4) -- node[left] {$\scriptstyle \vec{a}_1$} (v3);
      \path[omissis] (v4) edge [bend right=80] node {} (v5);
      \draw[arrow] (v5) -- node[right] {$\scriptstyle \cev{a}_1$}(v6);
      \draw[arrow] (v7) -- (v6);
      \draw[omissis] (v8) -- (v7);
      \draw[arrow] (v0) -- node[above right=-3pt ] {$\scriptstyle \vec{a}_p$} (v8);
    \end{tikzpicture}
  \end{center}
  
  For a vertex $i$ in $Q$, write $\nccw(i)$ for the next vertex of $Q$ counterclockwise from $i$ and write $\sccw(i)$ for the next source or sink of $Q$ counterclockwise from $i$.  
  Define $\ncw(i)$ and $\scw(i)$ similarly by looking clockwise from $i$.  
  Note that $\nccw(h(\vec{a}_\ell))=t(\vec{a}_\ell)$ and $\ncw(h(\cev{a}_\ell))=t(\cev{a}_\ell)$.  
  Also note that when $i$ is a source (resp. a sink) in $Q$, both $\sccw(i)$ and $\scw(i)$ will be sinks (resp. sources) in $Q$.  
  \saySS{No notation change was made after this point}

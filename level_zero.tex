...
An $n\times n$ matrix $B$ is said to be \newword{skew-symmetrizable} if it is related to a skew-symmetric matrix through multiplication by a diagonal matrix.
An \newword{exchange matrix} $\tilde B$ is any $m\times n$ integer matrix (with $m\geq n$)  whose square submatrix $B$ formed by the first $n$ rows is skew-symmetrizable.

Two exchange matrices $\tilde B$ and $\tilde B'$ related by \newword{mutation} at $k \in \{1,\dotsc,n\}$ if their entries are related by  
\begin{equation}\label{eq:matrix mutation}
  \tilde B'_{ij} = \begin{cases}
  -\tilde B_{ij} & i = k \text{ or } j = k\\
  \tilde B_{ij} + \sgn(\tilde B_{ik})[\tilde B_{ik}\tilde B_{kj}]_+ & \text{otherwise.}
  \end{cases}
\end{equation}
Here and elsewhere we write $[a]_+$ for $\max(a,0)$ and $\sgn(a) \in \{\pm 1,0\}$ for the sign of $a$ where $\sgn(a)=0$ for $a=0$. 

To define the (geometric type) cluster algebra $\cA_{\tilde B}$ associated to an $m \times n$ exchange matrix $\tilde B$ we begin with an infinite $n$-ary tree $\TT$. 
Label its edges by $\{1,\dotsc,n\}$ so that the edges incident to any given vertex have distinct labels. 
For a fixed root $t_0 \in\TT$, assign a matrix $\tilde B^t$ to each $t \in\TT$ so that $\tilde B^{t_0} = \tilde B$, and if $t, t' \in\TT$ are joined by an edge labeled $k$, then $\tilde B^t$ and $\tilde B^{t'}$ are related by mutation at $k$. 

Let $\cF$ denote the field of rational functions in formal variables $x_{1}\,\dotsc,x_{m}$ with coefficients in $\kk$. 
The \newword{cluster variables} $$\{ x_{i;t}: i \in \{1,\dotsc,m\}, t \in\TT\} \subset \cF$$ are defined recursively as follows. 
The initial cluster variables $x_{i;t_0}$ are taken to be the generators $x_i$. 
If $t,t' \in T_0$ are joined by an edge labeled $k$, then $x_{i;t} = x_{i;t'}$ for $i \neq k$, and $x_{k;t}$, $x_{k;t'}$ are related by the \newword{exchange relation} 
\[
  x_{k;t}x_{k;t'} 
  = 
  \prod_{\tilde B^t_{ik}>0}x_{i;t}^{\tilde B^t_{ik}} 
  + 
  \prod_{\tilde B^t_{ik}<0}x_{i;t}^{-\tilde B^t_{ik}}.
\]
The variables $x_{n+1;t},\dotsc, x_{m;t}$ do not depend on $t$ hence are referred to as \newword{frozen variables}. 

\begin{definition}
  \emph{[FZ]} The \newword{cluster algebra} $\cA_{\tilde B}$ is the $\kk$-subalgebra of $\cF$ generated by the set of all cluster variables.
\end{definition}
...
The terminology is justified by the skew-symmetric case: a skew-symmetric matrix $B$ can be recovered from the quiver $Q := Q_B$ whose vertices are indexed by the columns of $B$ and which has $[-B_{ij}]_+$ arrows to vertex $i$ from vertex $j$; the skew-symmetric matrix is acyclic exactly when this quiver has no oriented cycles. 
The cluster algebra associated to a matrix whose  acyclic principal part is especially well-behaved: it is finitely generated as a $\kk$-algebra and 
%\sayHW{Check reference}\sayDR{It's correct.} 
coincides with its upper cluster algebra [BFZ].
% (as such, we will not need to recall the latter notion explicitly).
\saySS{Rephrase this once we decide if we are going to use ``principal part'' or not}
% is acyclic if there is a permutation $\sigma \in S_n$ such that $B_{\sigma_i \sigma_j} \geq 0$ whenever $i < j$. Note that there is a map from skew-symmetric matrices to finite quivers: to such a matrix $B$ we associate the quiver $Q := Q_B$ whose vertices are indexed by $\{1,\dotsc,n\}$, and there are $[-B_{ij}]_+$ arrows from vertex $i$ to vertex $j$. If $B$ is skew-symmetric it is acyclic exactly when $Q$ has no oriented cycles.


%We say a skew-symmetrizable $n \times n$ matrix $B$ is acyclic if there is a permutation $\sigma \in S_n$ such that $B_{\sigma_i \sigma_j} \geq 0$ whenever $i < j$. Note that there is a map from skew-symmetric matrices to finite quivers: to such a matrix $B$ we associate the quiver $Q := Q_B$ whose vertices are indexed by $\{1,\dotsc,n\}$, and there are $[-B_{ij}]_+$ arrows from vertex $i$ to vertex $j$. If $B$ is skew-symmetric it is acyclic exactly when $Q$ has no oriented cycles. In general, the cluster algebra associated to a matrix with acyclic principal part is finitely generated as a $\kk$-algebra and %\sayHW{Check reference}\sayDR{It's correct.} 
...
%If $B$ is skew-symmetric (as opposed to just skew-symmetrizable), it is equivalent to the data of a quiver 
...
For each $1 \leq i \leq n$ and $t \in\TT$ there is an auxiliary pair of integer vectors depending only on the principal part of $B$, the \newword{$\bfc$-vector} $\bfc_{i;t} \in \ZZ^n$ and the \newword{$\bfg$-vector} $\bfg_{i;t} \in \ZZ^n$. We write $C_t^B$ and $G_t^B$ for the matrices whose $i$th columns are $\bfc_{i;t}$ and $\bfg_{i;t}$, respectively.
To define $C_t^B$, consider the $2n \times n$ matrix $\tilde B^{t_0}$ whose principal part equals the principal part of $B$ and whose bottom $n \times n$ block is the identity matrix.
For $t\in\TT$, compute $\tilde B^t$ by matrix mutations \eqref{eq:matrix mutation} from $\tilde B^{t_0}$, then $C_t^B$ is defined to be the bottom $n \times n$ block of $\tilde B^t$. 

Given the matrix of $\bfc$-vectors at a given $t\in\TT$, we may define the matrix of $\bfg$-vectors at the same vertex $t$ by
\[
  (G_t^B)^T = (C_t^{-B^T})^{-1}.
\]
Note that ths is not the the standard definition, but the above characterization, due to \cite[Theorem 1.2]{NZ12}, is what we will use in Section \ref{sec:exchange}; the equivalence of the two is guaranteed by sign coherence [GHKK??].
\saySS{maybe we do not need to say this}
...
\saySS{This is Conjecture 7.10 (1) in CA4 and conjecture 1.5 in quiver with potentials (proved there for skew-symmetric); this also follows immediately from the fact that cluster variables are in the theta basis and tetha basis elements are parametrized by g-vectors.}
...
\saySS{Do we believe this result?}
...
We will be interested in the following two $3n\times n$ extensions of $B$:
\begin{equation}
  B_{\bullet\bullet}
  :=
  \left[
  ...
  \right]
\end{equation}
where $1$ denotes the identity matrix and we apply $[\,\cdot\,]_+$ entrywise. 
We refer to them as the \newword{double principal} and \newword{double Euler} extension of $B$ respectively. 
\saySS{Notation here is temporary: we have two ``doubled'' things so $B_{dbl}$ seemed ambiguous.}
\saySS{The name for $B_{EE}$ comes from the fact that $1-[B]_+$ is the Euler matrix for $-B$. Unfortunately I can't do anything about the sign: live is tough} 

%The Coxeter element is $c = s_{\sigma_1} \cdots s_{\sigma_n}$, where $\sigma \in S_n$ is any permutation for which $\sigma_i > \sigma_j$ whenever there is an arrow from $i$ to $j$ in $Q_B$. Conversely, any Coxeter element $c$ arises in this way from a unique skew-symmetrizable matrix, which we write as $B_c$. \sayHW{We are deprecating $Q_B$, and possibly also $B_c$}.
...
For example, with respect to such a factorization it is clear that for any dominant weight $\lambda \in \widehat{P}^+$, we have $\Delta_\lambda(g) = h^\lambda$. %\sayDR{This seems to be the first use of this notation for the evaluation of a weight on an element of the Cartan subgroup, we should define it somewhere above.} 
...
\]
Our goal is to identify this with the evaluation $x_{V_{[k,\ell]}}(g)$ of the cluster character from \eqref{eq:regular cluster characters}, where the functions $x_i,\hat y_j\in\CC[G^{c,c^{-1}}]$ are determined by the initial cluster \eqref{eq:initialcluster}.
...
We consider the case where the first source in $[k,\ell]$ comes before the first sink (i.e. $o_1 < i_1$ in $[k,\ell]$), the opposite case following by symmetry.\sayDR{I need to fill in more details.} Recall that in this case
\[S_{[k,\ell]} = [k,o_1-1] \cup [i_1,o_2-1] \cup \cdots \cup [i_m,\ell].\]
...
Write $P_0\in\cP_{[k,\ell]}$ for the trivial collection of pairwise disjoint paths in which the path at level $j\in S_{[k,\ell]}=\partial_{in}P_0$ does not leave the level $j$ horizontal edge.
Note that $wt(P_0)=h_{k-1}h_\ell^{-1}\prod_{r=1}^m (h_{o_r}h_{i_r}^{-1})$.  
We will show how to modify $P_0$ to obtain a unique collection of paths $P\in\cP_{[k,\ell]}$ with $\beta(P)=E$ in such a way that $P$ clearly contains pairwise disjoint paths, along the way we also compute $wt(P)$ for these collections.  Note that since each $P\in\cP_{[k,\ell]}$ has the same starting and ending set of vertices $S_{[k,\ell]}$, it suffices to only describe the upward steps along bridges labeled $t_i$ for $i\in[1,n]$ since the downward steps along bridges labeled $t_i$ for $i\in[-n,-1]$ will be mirror to these upward steps.  
...
\begin{itemize}
  ...
  Otherwise, for each $j\in[i_{r-1},o'_r]$ the path staying on level $j$ will now traverse its corresponding bridge labeled $t_j$ and by part (1) of Lemma~\ref{lem:phi} the resulting collection $P'_r$ will remain pairwise disjoint.  
  In the latter case, the weight of $P_{r-1}$ transforms as follows: 
  \[wt(P'_r)=wt(P_{r-1})\cdot h_{i_{r-1}-1}^{-1}h_{i_{r-1}}h_{o'_r}h_{o'_r+1}^{-1}\prod_{j\in[i_{r-1},o'_r]}(t_jt_{-j}).\]
  \item Let $o''_r\in[o_r,i_r]$ be minimum so that $[o''_r,i_r]\subset E$.  
  If no such $o''_r$ exists (i.e. $i_r\notin E$) let $P_r=P'_r$.  
  Otherwise, the path staying on level $i_r$ will now traverse every bridge $t_j$ for $j\in[o''_r,i_r]$ and following part (2) of Lemma~\ref{lem:phi} the resulting collection $P_r$ will remain pairwise disjoint since $[o''_r,i_r]\cap\partial_{in}P'_r=\{i_r\}$ and when $o''_r=o_r$ (i.e. $o_r\in E$) we have $o_r-1\in\beta(P'_r)$.
  In the latter case, the weight of $P'_r$ transforms as follows: 
  \[wt(P_r)=wt(P'_r)\cdot h_{o''_r-1}^{-1}h_{o''_r}h_{i_r}h_{i_r+1}^{-1}\prod_{j\in[o''_r,i_r]}(t_jt_{-j}).\]
\end{itemize}
Then $P=P_{m+1}\in\cP_{[k,\ell]}$ is the desired collection of pairwise disjoint paths with $\beta(P)=E$.

\sayHW{Things not addressed yet: what about the other tube?}
...
\subsection{Affine Tubes}\sayDR{Clean up this section.}
  ...
  \begin{lemma}\sayDR{This should be translated into our current notation and placed in an appropriate location, maybe somewhere near Lemma~\ref{lemma:coefficients_values}}

...
\begin{equation}\label{eq:matrix mutation}
  ...
  \end{cases}
\end{equation}
%\saySS{I changed $B_{ij}$ into $b_{ij}$ for uniformity with Cartan matrix notation}
\sayDR{Are tildes necessary here?  They seem heavy.}\sayHW{I don't love them}
...
Fix a root vertex\sayDR{$\backslash$newword?}\sayHW{I don't think so: here root vertex is not a new word we're defining} $t_0 \in\TT$.  Assign an exchange matrix $\widetilde B^t$ to each $t \in\TT$ so that:
\begin{itemize}
...
The terminology is justified by the skew-symmetric case: the data of a skew-symmetric matrix $B$ is the same as that of a quiver $Q := Q_B$ with vertices indexed by the columns of $B$ and $[-B_{ij}]_+$ arrows to vertex $i$ from vertex $j$; the skew-symmetric matrix is acyclic exactly when this quiver has no oriented cycles. We say an exhange matrix is acyclic if its principal part is. 
...
Conversely, any acyclic skew-symmetrizable matrix can be obtained from a Cartan matrix and Coxeter element $c$ by assigning signs to the off-diagonal entries according to $c$.
...
With the preceding results in hand, we are ready to describe the cluster variables of $\cA_{\Bdp}$ attached to the acyclc belt.

\saySS{Is this a theorem or a proposition?}
\begin{theorem}
  \label{thm:vars_and_rels_in_bipartite_belt}
  The preprojective and postjective cluster variables of $\cA_{\Bdp}$ are parametrized by the sets of $\bfg$-vectors
  \[
...
\end{proof}
\begin{remark}
  As noted before, to obtain the exchange relations in the cluster algebra with principal coefficients $\cA_{\Bpr}$ it suffices to set $z_{-1},\dotsc,z_{-n}$ to one in the relations of \Cref{thm:vars_and_rels_in_bipartite_belt}. 
\end{remark}

...
\subsection{Affine Tubes}
  ...
  \end{itemize}
  Notice that $\overrightarrow{\rep}Q$ is generated by exactly $p$ indecomposable representations while $\overleftarrow{\rep}Q$ is generated by exactly $q$ indecomposable representations.  Indeed, the generators of $\overrightarrow{\rep}Q$ and $\overleftarrow{\rep}Q$ can conveniently be labeled by the clockwise and counterclockwise arrows respectively.  More precisely, we will label their dimension vectors as follows:
  \begin{equation}
    \vec{\beta}_{1,\ell}=\begin{cases}\alpha_i & \text{if $s(\vec{a}_\ell)=i$ is not a source in $Q$}\\\alpha_{i_t}+\cdots+\alpha_{i_{t+1}} & \text{if $s(\vec{a}_\ell)=i_t$ is a source in $Q$}\end{cases}\!\!\Bigg\}\quad\text{for $1\le\ell\le p$}
  \end{equation}
  and
  \begin{equation}
    \cev{\beta}_{1,\ell}=\begin{cases}\alpha_i & \text{if $t(\cev{a}_\ell)=i$ is not a sink in $Q$}\\\alpha_{i_s}+\cdots+\alpha_{i_{s+1}} & \text{if $t(\cev{a}_\ell)=i_s$ is a sink in $Q$}\end{cases}\!\!\Bigg\}\quad\text{for $1\le\ell\le q$.}
  \end{equation}
  This also gives a labeling of the generators of $\overrightarrow{\rep}Q$ and $\overleftarrow{\rep}Q$ respectively as $\vec{M}_{1,\ell}$ for $1\le\ell\le p$ and $\cev{M}_{1,\ell}$ for $1\le\ell\le q$.  The coindices of these representations can be described as follows:
  \begin{equation}
    \vec{\lambda}_{1,\ell}=\begin{cases}\omega_i-\omega_{i+1} & \text{if $s(\vec{a}_\ell)=i$ is not a source in $Q$}\\\omega_{i_{t+1}}-\omega_{i_{t+1}+1} & \text{if $s(\vec{a}_\ell)=i_t$ is a source in $Q$}\end{cases}\!\!\Bigg\}\quad\text{for $1\le\ell\le p$}
  \end{equation}
  and
  \begin{equation}
    \cev{\lambda}_{1,\ell}=\begin{cases}\omega_i-\omega_{i-1} & \text{if $t(\cev{a}_\ell)=i$ is not a sink in $Q$}\\\omega_{i_s}-\omega_{i_s-1} & \text{if $t(\cev{a}_\ell)=i_s$ is a sink in $Q$}\end{cases}\!\!\Bigg\}\quad\text{for $1\le\ell\le q$.}
  \end{equation}
  ...
  \end{equation}
  Similarly the coindices of these representations can be computed as
  \begin{align}
    \vec\lambda_{k,\ell}
    &=\sum\limits_{m=1}^k\vec\lambda_{1,\ell+m-1}\quad\text{for $1\le k\le p-1$ and $1\le\ell\le p$}\\
    &=\begin{cases}\omega_i-\omega_{i+1} & \text{if $s(\vec{a}_\ell)=i$ is not a source in $Q$}\\\omega_{i_{t+1}}-\omega_{i_{t+1}+1} & \text{if $s(\vec{a}_\ell)=i_t$ is a source in $Q$}\end{cases}\!\!\Bigg\}
  \end{align}
  and
  \begin{align}
    \cev\lambda_{k,\ell}
    &=\sum\limits_{m=1}^k\cev\lambda_{1,\ell+m-1}\quad\text{for $1\le k\le q-1$ and $1\le\ell\le q$}\\
    &=\begin{cases}\omega_i-\omega_{i-1} & \text{if $t(\cev{a}_\ell)=i$ is not a sink in $Q$}\\\omega_{i_s}-\omega_{i_s-1} & \text{if $t(\cev{a}_\ell)=i_s$ is a sink in $Q$}\end{cases}\!\!\Bigg\}
  \end{align}

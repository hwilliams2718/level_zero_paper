...
In this section we present straightforward gerenalizations of some of the results in \cite[sections 8-10]{FZ07} to the acyclic case.
...
\saySS{The case $A$ of finite type is secretly covered by \cite{YZ08}. We can cover it if we use $w_0$ in place of $c^\infty$. It is not clear to me that it is worth doing even though it will make life more uniform. Maybe we should add a comment here.}
...
We will concern ourself with a specific sequence of seeds in the principal coefficients cluster algebra $\cAb(B_c)$.
...
\begin{equation}
  ...
  \cdots
\end{equation}
\saySS{We should probably comment on the fact that all these nodes of the exchange graph are distict. It follows from the propositions we are about to prove but one might wonder whether the argument is circular here.}
Following \cite{FZ02}, we denote by $\langle m \rangle$ the element of $[1,n]$ congruent to $m$ modulo $n$.
The edge immediately to the left of $t_m$ is then labeled by $\langle m \rangle$.
...
\begin{proposition}
  ...
  \begin{equation}
    ...
    =
    \begin{cases}
      c^{m\sslash n}s_1\cdots s_{\langle m \rangle} \alpha_i & \text{if $m>0$}\\
      \alpha_i & \text{if $-n\leq m \leq 0$ and  $i\leq n+m$} \\
      -\alpha_i & \text{if $-n\leq m \leq 0$ and $i>n+m$} \\
      -c^{2+(m+1)\sslash n}s_n\cdots s_{\langle m+1 \rangle} \alpha_i & \text{if $m <-n$}\\
    \end{cases}
  \end{equation}
Where $m\sslash n$ is a shorthand for $(m-\langle m \rangle)/n$.
Moreover, whenever $m > 0 $ or $m < -n$, the vector $\gamma_{m,\langle m\rangle}$ is negative and the vector $\gamma_{m,\langle m+1\rangle}$ is positive.
\end{proposition}

\begin{remark}
  If $m>0$ then $c^{m\sslash n}s_1\cdots s_{\langle m \rangle}$ is the prefix of $c^\infty$ of length $m$.
  Similarly, if $m<-n$, $c^{2+(m+1)\sslash n}s_n\cdots s_{\langle m+1 \rangle}$ is the prefix of $c^{-\infty}$ of length $-m-n$.
\end{remark}

\begin{proof}
  Let's begin by settling the claim about signs. 
  When $m>0$ the root $c^{m\sslash n}s_1\cdots s_{\langle m \rangle} \alpha_{\langle m+1\rangle}$ is positive because $c^{m\sslash n}s_1\cdots s_{\langle m \rangle}  s_{\langle m+1 \rangle}$ is reduced (it is a prefix of $c^\infty$, cf. \cite{Spe09}) while the root $c^{m\sslash n}s_1\cdots s_{\langle m \rangle} \alpha_{\langle m\rangle}$ is negative because $c^{m\sslash n}s_1\cdots s_{\langle m \rangle}  s_{\langle m \rangle}$ is not reduced.
  Similarly, when $m<-n$ the root $-c^{2+(m+1)\sslash n}s_n\cdots s_{\langle m+1 \rangle} \alpha_{\langle m+1\rangle}$ is positive and the root $-c^{2+(m+1)\sslash n}s_n\cdots s_{\langle m+1 \rangle} \alpha_{\langle m\rangle}$ is negative.

  We begin by observing that the nodes we are considering lie along a \emph{sink/source adapted sequence}. 
  In particular the entries of the $B$-matrix $B_m$ at $t_m$ equal, in absolute value, the entries of $B_0$. 
  Moreover the $\langle m \rangle$-th row of $B_m$ contains only non-positive entries and the $\langle m+1 \rangle$-th row only non-negative entries.

  Suppose $m>0$; we proceed by induction

  In view of \cite[Proposition 1.3]{NZ12}
  We need to consider three cases the easiest of which is $-n \leq m \leq 0$. 
\end{proof}
...
\begin{proposition}
  ...
  \begin{equation}
    ...
    =
    \begin{cases}
      c^{m\sslash n}s_1\cdots s_{\langle m \rangle} \omega_i & \text{if $m>0$}\\
      \omega_i & \text{if $-n\leq m \leq 0$ and  $i\leq n+m$} \\
      -\omega_i & \text{if $-n\leq m \leq 0$ and $i>n+m$} \\
      -c^{2+(m+1)\sslash n}s_n\cdots s_{\langle m+1 \rangle} \omega_i & \text{if $m <-n$}\\
    \end{cases}
  \end{equation}
\end{proposition}
...
In view of how simple reflections act on fundamental weights the expressions of the $\bfg$-vectors $\lambda_{m,i}$ can be further explicitated as follows:
\begin{equation}
  \lambda_{m,i}
  =
  \begin{cases}
    c^{m\sslash n} \omega_i & \text{if $m>0$ and $i > \langle m \rangle$}\\
    c^{1+ m\sslash n} \omega_i & \text{if $m>0$ and $i \leq \langle m \rangle$}\\
    \omega_i & \text{if $-n\leq m \leq 0$ and  $i\leq n+m$} \\
    -\omega_i & \text{if $-n\leq m \leq 0$ and $i>n+m$} \\
    -c^{2+(m+1)\sslash n} \omega_i & \text{if $m <-n$ and $i < \langle m+1 \rangle$}\\
    -c^{1+(m+1)\sslash n} \omega_i & \text{if $m <-n$ and $i \geq \langle m+1 \rangle$}\\
  \end{cases}
\end{equation}
...
\end{corollary}
\begin{proof}
  The relations are obvious once we recall that mutations happen along a source/sink adapted sequence so that, at $t_m$, the $\langle m \rangle$-th colum of $B_m$ contains only non-negative entries and the $\langle m+1 \rangle$-th column only non-positive entries. 
  At the same time, for $m>0$ or $m < -n$, the $\bfc$-vector $\gamma_{m,\langle m+1\rangle}$ is positive and  the $\bfc$-vector $\gamma_{m,\langle m\rangle}$ is negative because prefixes of $c^{\pm\infty}$ are reduced.
  \saySS{Maybe more words here?}
  These considerations are enough to establish the first two sets of relations. 
  The third follows by direct inspection of the seeds $t_{-n},\dots, t_0$.
\end{proof}

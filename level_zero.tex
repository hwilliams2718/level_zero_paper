...
\end{lemma}
Define the $2n\times n$ matrix $\tilde B_{k,m}=\mu_k\mu_{k-1}\cdots\mu_1(\mu_n\cdots\mu_1)^m\left[\begin{array}{c} B_c\\ I_n\end{array}\right]$.  We identify the root lattice with $\ZZ^n$.
\begin{lemma}
  The $c$-vectors of $\tilde B_{k,m}$ are given by $\gamma^{(k,m)}_j=c^m s_1\cdots s_k\alpha_j$.  Moreover, the principal part of column $k$ in $\tilde B_{k,m}$ is weakly positive while the principal part of column $k+1$ is weakly negative (for $k=n$ we take $k+1$ to be $1$).
\end{lemma}
\begin{proof}
  We work by induction on the length of the mutation sequence.  

  Claim: $\gamma^{(k,m)}_j=s_{\gamma^{(k-1,m)}_k}\big(\gamma^{(k-1,m)}_j\big)$\\

  Proof: Since the pairing $(\cdot,\cdot)$ is $W$-invariant we have $(\gamma^{(k-1,m)}_j,\gamma^{(k-1,m)}_k)=(\alpha_j,\alpha_k)$ so that
  \begin{align*}

...
We will study generalized minors by modeling them with weighted graphs on the cylinder. A graph is weighted if all of its edges are assigned elements of a fixed ring; below this will be the Laurent polynomial ring $\CC[t_0^{\pm1},\dotsc,t_n^{\pm1},u_0^{\pm1},\dotsc,u_n^{\pm1}]$.

\begin{definition}
Given $c = s_{\sigma_0} \dotsc s_{\sigma_n}$, let $\net_c$ be the following directed, weighted graph on the cylinder. We describe it as the concatenation of subgraphs on cyclinders associated to the factors in the decomposition $$g=x_{-\sigma_0}(t_{-\sigma_0})\cdots x_{-\sigma_n}(t_{-\sigma_n}) hx_{\sigma_n}(t_{\sigma_n}) \cdots x_{\sigma_0}(t_{\sigma_0}).$$\sayHW{This definition will require a picture}

Each subcylinder has $n+1$ noncrossing edges running from one boundary component to the other, and labeled by $\ZZ_{n+1}$ compatibly with their cyclic ordering. We refer to these as horizontal edges, and each subcylinder has one additional edge we refer to as a vertical edge.

In the subcylinder associated to $x_i(t_i)$ the vertical edge runs from the $i^{th}$ horizontal edge to the $(i - 1)^{st}$ horizontal edge and this vertical edge is weighted by $t_i$.  Analogously, the vertical edge in the subcylinder associated to $x_{-i}(t_{-i})$ runs from the $(i - 1)^{st}$ horizontal edge to the $i^{th}$ horizontal edge and this vertical edge is weighted by $t_{-i}$.  Each horizontal edge of a subcylinder has weight $1$.  
...
\begin{tikzpicture}
  ...
  \node at (-0.6,-0.875) {$\scriptstyle h_i^{-1}$};
  \draw[-] (-1.245,0.2) -- (0.255,0.2);
...
\end{center}
\begin{center}
\begin{tikzpicture}[decoration={markings,mark=at position 0.85 with {\arrow{stealth}}]
  \node[below] at (0,-2) {$x_i(t_i)$};
  \draw (0.75,0) ellipse (0.5 and 2);
  %\draw (-1,0) ellipse (0.5 and 1);
  \draw (-0.75,2) arc (90:270:0.5 and 2);
  ...
  \draw[-] (1.25,0) -- (0.25,0);
  \draw[dashed] (-0.25,0) -- (0.21,0);
  \draw[draw=none,postaction=decorate] (1.25,0) -- (-0.25,0);
  \node[right] at (1.25,0) {$\scriptstyle 0$};
  \draw[-] (1.2,-0.85) -- (0.3,-0.85);
  \draw[dashed] (-0.3,-0.85) -- (0.3,-0.85);
  \draw[draw=none,postaction=decorate] (1.2,-0.85) -- (-0.3,-0.85);
  \node[right] at (1.2,-0.85) {$\scriptstyle n$};
  \draw[-] (1.015,-1.7) -- (0.49,-1.7);
  \draw[dashed] (-0.49,-1.7) -- (0.45,-1.7);
  \draw[draw=none,postaction=decorate] (1.015,-1.7) -- (-0.49,-1.7);
  \node[right] at (1.015,-1.7) {$\scriptstyle n-1$};
  \draw[-] (-1.225,-0.65) -- (0.275,-0.65);
  \draw[draw=none,postaction=decorate] (0.275,-0.65) -- (-1.225,-0.65);
  \node[right] at (0.275,-0.65) {$\scriptstyle i$};
  \draw[postaction=decorate] (-0.475,-0.65) arc (199:174.2:0.5 and 2);
  \node at (-0.7,-0.225) {$\scriptstyle t_i$};
  \draw[-] (-1.245,0.2) -- (0.255,0.2);
  \draw[draw=none,postaction=decorate] (0.255,0.2) -- (-1.245,0.2);
  \node[right] at (0.255,0.2) {$\scriptstyle i-1$};
  %\draw[postaction=decorate] (-0.495,0.2) arc (174.2:199:0.5 and 2);
  %\draw[-] (-1.18,1.05) -- (0.32,1.05);
  %\node[right] at (0.32,1.05) {$\scriptstyle i-1$};
...
\end{theorem}
\begin{proof}
Let $\cP = \{P: S_{[p,q]} \to S_{[p,q]}\}$ denote the set of tuples of noncrossing paths in $\net_c$ whose sets of start and end vertices are $S_{[p,q]}$. 
By Proposition \ref{prop:minorsfrompaths}, for a generic element $g = $ the evaluation $\Delta_{\omega_{S_{[p,q]}}}(g)$ is the weighted sum 
\[
\Delta_{S_{[p,q]}}(g) = \sum_{P \in \mathcal{P}} wt(P).
\]
...
\]
We will prove the theorem by constructing a bijection $\cP \cong \cE$, then show it is compatible with the monomials appearing in each expression.
...
Let $\phi(P) \subset \ZZ_{n+1}$ denote the set of indices $j$ such that $wt(P)$ depends nontrivially on $t_j$. Recall that $t_j$ appears as the weight of a unique edge in $\net_c$, so equivalently $\phi(P)$ records which of these edges are traversed by a path in $P$. We claim that $\phi(P)$ is in fact a subset of $[p,q]$, is target-closed in $[p,q]$, and that the assignment $P \mapsto \phi(P)$ defines a bijection $\cP \cong \cE$.


As in Definition \ref{def:Spq}, we let $k_1 < \cdots < k_m$, $c_1 < \cdots < c_m$ denote the labels of the sinks and sources in $[p,q]$. We first consider the case where the first sink in $[p,q]$ comes before the first source (i.e. $k_1 < c_1$ in $[p,q]$). 
\end{proof}

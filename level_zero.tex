...
\]
\saySS{should we add ``associated to $\widetilde B$'' ore something?}\sayDR{maybe add ``of $\cA_{\widetilde B}$''?}
...
\begin{definition}
  ...
  \saySS{again ``associated to $\widetilde B$''?}
\end{definition}
...
The basic role of $\bfg$-vectors is that they provide natural labels of cluster variables: two cluster variables $x_{i;t}$ and $x_{j;t'}$ coincide in $\cA_{\widetilde B}$ if and only if $\bfg_{i;t}$ and $\bfg_{j;t'}$ coincide in $\ZZ^n$.

...
When $Q$ is of classical ADE type and $G$ a simple algebraic group, Yang and Zelevinsky showed these functions have an interpretation in terms of the representation theory of $G$: they are restrictions of generalized minors. 
These are functions on $G$ labeled by a group representation $V$ together with a choice of extremal one-dimensional weight space $V_\lambda$. 
Their value on $g \in G$ is the ratio of any nonzero $v \in V_\lambda$ with the projection of $gv \in V$ to $V_\lambda$. 
Our main result is that cluster variables in $\cA_Q$ are also realized as generalized minors on $G^{c,c^{-1}}$ when $Q$ is not necessarily finite type, and that moreover the resulting correspondence between quiver representations and group representations intertwines the classifications described above.
...
An \newword{exchange matrix} $\widetilde B$ is any $m\times n$ integer matrix (where $m\geq n$) with skew-symmetrizable submatrix $B$ formed by its first $n$ rows.
We will refer to such a $\widetilde B$ as an \newword{extension} of $B$.

Two exchange matrices $\widetilde B$ and $\widetilde B'$ are related by \newword{mutation} at $k \in \{1,\dotsc,n\}$ if their entries satisfy 
\begin{equation}\label{eq:matrix mutation}
  \widetilde b'_{ij} = \begin{cases}
  -\widetilde b_{ij} & i = k \text{ or } j = k\\
  \widetilde b_{ij} + \sgn(\widetilde b_{ik})[\widetilde b_{ik}\widetilde b_{kj}]_+ & \text{otherwise.}
  \end{cases}
\end{equation}
\saySS{I changed $B_{ij}$ into $b_{ij}$ for uniformity with Cartan matrix notation}\sayDR{Are tildes necessary here?  They seem heavy.}
...
To define the (geometric type) cluster algebra $\cA_{\widetilde B}$ associated to an $m \times n$ exchange matrix $\widetilde B$, we begin with an infinite $n$-ary tree $\TT$ with edges labeled by $\{1,\dotsc,n\}$ so that the edges incident to any given vertex have distinct labels.
Fix a root vertex\sayDR{$\backslash$newword?} $t_0 \in\TT$.  Assign an exchange matrix $\widetilde B^t$ to each $t \in\TT$ so that:
\begin{itemize}
...
\[
  \mathcal{X}_{\widetilde B} :=\big\{ x_{i;t}: i \in [1,m], t \in\TT\big\} \subset \cF
\]
...
\[
  x_{k;t}x_{k;t'} 
  = 
  \prod_{\widetilde b^t_{ik}>0}x_{i;t}^{\widetilde b^t_{ik}} 
  + 
  \prod_{\widetilde b^t_{ik}<0}x_{i;t}^{-\widetilde b^t_{ik}}.
\]
...
For each $t \in\TT$ and $1 \leq i \leq n$ there is an auxiliary pair of integer vectors depending only on $B$, namely the \newword{$\bfc$-vector} $\gamma_{i;t} \in \ZZ^n$ and the \newword{$\bfg$-vector} $\lambda_{i;t} \in \ZZ^n$. 
\saySS{I renamed the $\bfc$- and $\bfg$-vectors to have uniform notation with section 3.1. There there are plenty of $c$'s floating around and it is probably sligthly better try to distinguish them. This notation emphasize that we are dealing with roots and weights even if it is not stated here yet.}\sayDR{Is $\gamma$ common notation for a root?  I am more familiar with $\alpha$.}
We write $C_t^B$ and $G_t^B$ for the matrices whose $i$th columns are $\gamma_{i;t}$ and $\lambda_{i;t}$, respectively.
To define $C_t^B$, consider the extension $\widetilde B_\bullet$ of $B$ by an identity matrix.
\saySS{I am temporarely using the bullet notation, we should fix this later}
For $t\in\TT$, compute $\widetilde B_\bullet^t$ by matrix mutations \eqref{eq:matrix mutation} from $\widetilde B_\bullet^{t_0}=\widetilde B_\bullet$, then $C_t^B$ is defined to be the bottom $n \times n$ submatrix of $\widetilde B_\bullet^t$. 
...
\begin{equation}
  ...
  (G_t^B)^T = (C_t^{-B^T})^{-1}.
\end{equation}
Note that this is not the the standard definition, but the above characterization, due to \cite[Theorem 1.2]{NZ12}, is what we will use in Section \ref{sec:bipartite_belt}; the equivalence of the two is guaranteed by sign coherence [GHKK??].
\saySS{maybe we do not need to say anything about this equivalence}
The basic role of $\bfg$-vectors is that they provide natural labels of cluster variables: two cluster variables $x_{i;t}$ and $x_{j;t'}$ coincide in $\cA_{\widetilde B}$ if and only if $\lambda_{i;t}$ and $\lambda_{i;t'}$ coincide in $\ZZ^n$.
\saySS{Citation Needed. This is Conjecture 7.10 (1) in CA4 and conjecture 1.5 in quiver with potentials (proved there for skew-symmetric); this also follows immediately from the fact that cluster variables are in the theta basis and tetha basis elements are parametrized by g-vectors.}
...
\saySS{I just realized that this definition of bipartite belt is wrong: we do not take care of commuting mutations here. Somehow we should improve on this definition by sayng that we allow commutation relations but not braid relations.}
\begin{equation}
  \cdots
  \dashname{\sigma_n}
  t_{-n}
...
The goal of this section is to realize geometrically the ``doubled pricipal coefficients'' cluster algebra $\cA_{\widetilde B_{\doublebullet}}$, given by the $3n\times n$ extensions $\widetilde B_{\doublebullet}$ of $B$ by two identity matrices, in terms of the derived Kac-Moody group $G$ associated to $A$. 
...
\begin{theorem}
  \label{thm:coordring}
  Let $B$ be an acyclic skew-symmetrizable matrix, $G$ the derived Kac-Moody group of its cartan companion $A$, and $c$ the associated Coxeter element. 
  Then there is an isomorphism $\cA_{\widetilde B_{\doublebullet}} \cong \kk[G^{c,c^{-1}}]$ that identifies the initial cluster variables with the following restrictions of monomials in generalized minors
  \begin{equation}
...
\saySS{I changed $z_j$ to $z_{-j}$ if I am not mistaken these are the minors that are identically one on $L^{c,c^{-1}}$.}
In particular $\kk[L^{c,c^{-1}}]$ is the cluster algebra with principal coefficients associated to $B$. 
...
\saySS{Shall we move this to section 3?}
...
In particular the initial cluster variables of $\cA_{\widetilde B_{\doublebullet}}$ evaluates to $x_i(g)=h^{\omega_i}$.
...
In this section we prove that when $B$ is any acyclic skew-symmetrizable matrix, the identification $\cA_{\widetilde B_{\doublebullet}} \cong \kk[G^{c,c^{-1}}]$ of \Cref{thm:coordring} realizes preprojective and postinjective cluster variables as restrictions of some highest- and lowest-weight minors, respectively. 
...
We begin our analisys by computing $\bfc$-vectors will interpret them as coordinate vectors in the basis of simple roots for the root system of $A$ so that the $i$-th $\bfc$-vector at $t_0$ is $\alpha_i$.
\begin{lemma}
  ...
  \begin{equation}
    ...
    \end{cases}
  \end{equation}
  Where, as usual, $\left\lfloor a \right\rfloor$ is the biggest integer smaller or equal to $a$ while $\left\lceil a \right\rceil$ is the smallest integer greater or equal to $a$.
...
With the preceding results in hand, we are ready to describe the cluster variables of $\cA_{\widetilde B_{\doublebullet}}$ attached to the acyclc belt.

\saySS{Is this a theorem or a proposition?}
\begin{theorem}
  \label{thm:vars_and_rels_in_bipartite_belt}
  The preprojective and postjective cluster variables of $\cA_{\widetilde B_{\doublebullet}}$ are parametrized by the sets of $\bfg$-vectors
  \[
...
\begin{proof}
  ...
  The relations are then an obvious consequence of \Cref{lemma:c-vectors} once we observe that mutations happen along a source/sink adapted sequence so that, at $t_m$, the $\mod{m}$-th colum of $B^{t_m}$ contains only non-negative entries and the $\mod{m+1}$-th column only non-positive entries. 
\end{proof}
\begin{remark}
  As noted before, to obtain the exchange relations in the cluster algebra with principal coefficients $\cA_{\widetilde B_\bullet}$ it suffices to set $z_{-1},\dotsc,z_{-n}$ to one in the relations of \Cref{thm:vars_and_rels_in_bipartite_belt}. 
\end{remark}
...
%Since $(1-c)(\omega_i) = \beta_i^+$, if $1-c \in \End \widehat{P}_\RR$ were invrtible we could obtain Equation \ref{eq:coxeter_identity_omega} by applying $(1-c)^{-1}$ to Equation \ref{eq:coxeter_identity_beta}. In general, the kernel of $1-c$ coincides with the kernel of the natural projection $\widehat{P} \onto P$, which is nonzero except in finite type. 
...
Since $\iota$ preserves $B_+$ and $B_-$ but takes any lift of $w \in W$ to a lift of $w^{-1}$, it restricts to a biregular isomorphism between $G^{u,v}$ and $G^{u^{-1},v^{-1}}$. \saySS{Please check my math here: if an iso is defined on an open dense then it is birationa, correct?}\sayHW{You are correct, but here is a better argument for what we want.} Since a generic element of $G$ factors as $n_- h n_+$, with $n_- \in N_-$, $h \in H$, $n_+ \in N_+$, we have $\Delta_{\omega_j}(g) = \Delta_{-\omega_j}(g^\iota)$ for all $g \in G$. 

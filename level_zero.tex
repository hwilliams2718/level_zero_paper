...
\]
with $h\in H$ and $t_{\pm i} \in \CC$ (Cf. \cite[Proposition 3.2]{Wil13} were we prefer to place the tours part in center position using the commutation relations 
...
\end{proposition}
\begin{proof}
  In the ``opposite'' double Bruhat cell $G^{c^{-1},c}$ \cref{prop:projective_minors_identity} says that $\Delta_{c^{-k}\omega_i} \Delta_{c^{-k-1}\omega_i}$ equals
  \[
    \prod_{j<i}\Delta_{c^{-k}\omega_j}^{-a_{ji}}
    \prod_{j>i}\Delta_{c^{-k-1}\omega_j}^{-a_{ji}}
    +
    ...
    \right)^{[c^{-k}\beta_i^-:\alpha_j]}
  \]
  Recalling that $\Delta_\lambda^\mu(x)\Delta_{-\mu}^{-\lambda}(x^\iota)$ and that $\iota$ maps $G^{c^{-1},c}$ to $G^{c,c^{-1}}$, we get that $\Delta_{-c^{-k}\omega_i} \Delta_{-c^{-k-1}\omega_i}$ equals
  \[
    \prod_{j<i}\Delta_{-c^{-k}\omega_j}^{-a_{ji}}
    \prod_{j>i}\Delta_{-c^{-k-1}\omega_j}^{-a_{ji}}
    +

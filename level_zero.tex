...
\subsection{Affine Tubes}
  ...
  Let $\sigma\in\Sigma_{n+1}$ denote a permutation of the vertices of $Q$ such that $\sigma_n$ is a sink in $Q$ and $\sigma_i$ is a sink in $\mu_{\sigma_{i+1}}\cdots\mu_{\sigma_n} Q$ for $1\le i\le n$.  Let $c=s_{\sigma_0}\cdots s_{\sigma_n}$ denote the Coxeter element adapted to $Q$.
  \erase{%begin erase
  \begin{theorem}
  ...
  \begin{theorem}
    ...
    \begin{enumerate}
      \item For $1\le k\le p-1$ and $1\le\ell\le p$ we have
      \begin{align}
        &\Delta_{-\vec{\lambda}_{k,\ell}}(g)=\sum_{J\subset I}\left(\prod_{i\in J} t_i\prod_{i\in I\setminus J} u_i\left(\prod_{i=t(\vec{a}_\ell)^-}^{t(\vec{a}_\ell)-1}t_i+\sum_{m=t(\vec{a}_\ell)^-+1}^{t(\vec{a}_\ell)}u_{m-1}\prod_{i=m}^{t(\vec{a}_\ell)-1} t_i\right)^\delta\times\right.\\
        \nonumber&\times\left.\sum_{P\subset\bar{J}}\prod_{\substack{i:j^-<i<j^+\\j\in P}}t_i\prod_{i\in O\setminus P}u_i^{-1}\left(\prod_{j\in I^-(P)}\sum_{m=j^-+1}^ju_{m-1}\prod_{i=m}^{j-1} t_i\right)\left(\prod_{j\in I^+(P)}\sum_{m=j}^{j^+-1}u_{m+1}\prod_{i=j+1}^{m} t_i\right)\right)
      \end{align}
      where:
      \begin{itemize}
      ...
      \item For $1\le k\le q-1$ and $1\le\ell\le q$  we have
      \begin{align}
        &\Delta_{-\scev{\lambda}_{k,\ell}}(g)=\sum_{J\subset I}\left(\prod_{i\in J} t_i\prod_{i\in I\setminus J} u_i\left(\prod_{i=t(\scev{a}_\ell)+1}^{t(\scev{a}_\ell)^+}t_i+\sum_{m=t(\scev{a}_\ell)}^{t(\scev{a}_\ell)^+-1}u_{m+1}\prod_{i=t(\scev{a}_\ell)+1}^m t_i\right)^\delta\times\right.\\
        \nonumber&\times\left.\sum_{P\subset\bar{J}}\prod_{\substack{i:j^-<i<j^+\\j\in P}}t_i\prod_{i\in O\setminus P}u_i^{-1}\left(\prod_{j\in I^-(P)}\sum_{m=j^-+1}^ju_{m-1}\prod_{i=m}^{j-1} t_i\right)\left(\prod_{j\in I^+(P)}\sum_{m=j}^{j^+-1}u_{m+1}\prod_{i=j+1}^{m} t_i\right)\right)
      \end{align}
      where:
      \begin{itemize}
  ...
  \begin{theorem}\mbox{}
    \begin{enumerate}
      \item For $1\le k\le p-1$ and $1\le\ell\le p$ we have
      \begin{align}
        &x_{\vec{M}_{k,\ell}}=x^{\sum_{i\in O}\omega_i-\sum_{i\in I}\omega_i}\sum_{J\subset I}\prod_{i\in J}x_{i-1}^{-1}y_ix_{i+1}^{-1}\left(\sum_{m=t(\vec{a}_\ell)^-}^{t(\vec{a}_\ell)-1}x_m^{-1}x_{t(\vec{a}_\ell)}\prod_{i=m}^{t(\vec{a}_\ell)-1}y_i\right)^\delta\times\\
        \nonumber&\times\sum_{P\subset\bar{J}}\prod_{j\in P}\left(x_{j^-}x_j^{-2}x_{j^+}\prod_{i=j^-+1}^{j^+-1}y_i\right)\left(\prod_{j\in I^-(P)}\sum_{m=j^-+1}^{j-1}x_{m-1}^{-1}x_j\prod_{i=m}^{j-1}y_i\right)\left(\prod_{j\in I^+(P)}\sum_{m=j+1}^{j^+-1}x_jx_{m+1}^{-1}\prod_{i=j+1}^{m}y_i\right)
      \end{align}
      where:
      \begin{itemize}
        \item $I$ denotes the sinks in the support quiver $\vec{Q}_{k,\ell}$ of $\vec{M}_{k,\ell}$ read counterclockwise around $Q$,
      ...
      \item For $1\le k\le q-1$ and $1\le\ell\le q$ we have
      \begin{align}
        &x_{\scev{M}_{k,\ell}}=x^{\sum_{i\in O}\omega_i-\sum_{i\in I}\omega_i}\sum_{J\subset I}\prod_{i\in J}x_{i-1}^{-1}y_ix_{i+1}^{-1}\left(\sum_{m=t(\scev{a}_\ell)+1}^{t(\scev{a}_\ell)^+}x_m^{-1}x_{t(\scev{a}_\ell)}\prod_{i=m}^{t(\scev{a}_\ell)-1}y_i\right)^\delta\times\\
        \nonumber&\times\sum_{P\subset\bar{J}}\prod_{j\in P}\left(x_{j^-}x_j^{-2}x_{j^+}\prod_{i=j^-+1}^{j^+-1}y_i\right)\left(\prod_{j\in I^-(P)}\sum_{m=j^-+1}^{j-1}x_{m-1}^{-1}x_j\prod_{i=m}^{j-1}y_i\right)\left(\prod_{j\in I^+(P)}\sum_{m=j+1}^{j^+-1}x_jx_{m+1}^{-1}\prod_{i=j+1}^{m}y_i\right)
      \end{align}
      where:
      \begin{itemize}
  ...
  \begin{lemma}\mbox{}
    ...
    \begin{enumerate}
      ...
      \item $\Delta_{c\omega_i}^{\omega_i}(g)=\begin{cases}u_i^{-1}\prod_{j=i^-}^{i^+}t_j & \text{if $i$ is a sink in $Q$;}\\u_i^{-1}t_i & \text{if $i$ is a source in $Q$;}\\u_i^{-1}\prod_{j=i}^{i^+}t_j & \text{if $i$ is neither a source nor a sink in $Q$ but $i^+$ is a source;}\\u_i^{-1}\prod_{j=i^-}^it_j & \text{if $i$ is neither a source nor a sink in $Q$ but $i^-$ is a source.}\end{cases}$
      \item For $y_i=$
    \end{enumerate}
  \end{lemma}

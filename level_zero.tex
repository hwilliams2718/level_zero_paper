...
\saySS{should we add ``associated to $\tilde B$'' ore something?}
\[
  \{ x_{i;t}: i \in \{1,\dotsc,m\}, t \in\TT\} \subset \cF
\]
...
\begin{definition}
  ...
  \saySS{again ``associated to $\tilde B$''?}
\end{definition}

For each $1 \leq i \leq n$ and $t \in\TT$ there is an auxiliary pair of integer vectors depending only on $B$, the \newword{$\bfc$-vector} $\bfc_{i;t} \in \ZZ^n$ and the \newword{$\bfg$-vector} $\bfg_{i;t} \in \ZZ^n$. 
We write $C_t^B$ and $G_t^B$ for the matrices whose $i$th columns are $\bfc_{i;t}$ and $\bfg_{i;t}$, respectively.
To define $C_t^B$, consider the extension $B_\bullet$ of $B$ by an identity matrix.
\saySS{I am temporarely using the bullet notation, we should fix this later}
For $t\in\TT$, compute $B_\bullet^t$ by matrix mutations \eqref{eq:matrix mutation} from $B_\bullet^{t_0}=B_\bullet$, then $C_t^B$ is defined to be the bottom $n \times n$ block of $B_\bullet^t$. 
...
The basic role of $\bfg$-vectors is that they provide natural labels of cluster variables: two cluster variables $x_{i;t}$ and $x_{j;t'}$ coincide in $\cA_{\tilde B}$ if and only if $\bfg_{i;t}$ and $\bfg_{j;t'}$ coincide in $\ZZ^n$.
...
For such a cluster algebra we will concern ourself with a specific sequence of nodes (the \newword{acyclic belt}) in $\TT$ and the cluster variables associated to them.

More specifically, let $t_0$ be the node of $\TT$ containing the initial exchange matrix and $t_m$ the nodes given by the sequence
\begin{equation}
  \cdots
  \dashname{\sigma_n}
  t_{-n}
  ...
  \cdots
\end{equation}
it is straigthforward to observe that all the matrices $B^{t_m}$ are acyclic when $B^{t_0}$ is. 
\saySS{Shall we say that these are all?}
...
\saySS{Maybe here is a good place to justify our notation}
...
\saySS{This result should be revised in view of the understanding we recently gained on changing coefficients. It may also make sense to move it to the only place were we need it.}
...
\begin{theorem}
  \label{thm:coordring}\say[Group]{We need to think carefully about the change of coefficents, keeping in mind [Yang,Zelevinsky: p. 21] at the end of Sec. 4 and [Berenstein,Rupel: Lemma 6.8].}
...
In this section we prove that when $B$ is any acyclic skew-symmetrizable matrix, the identification $\cA_{\widetilde B_{\bullet\bullet}} \cong \kk[G^{c,c^{-1}}]$ of \Cref{thm:coordring} realizes preprojective and postinjective cluster variables as restrictions of highest- and lowest-weight minors, respectively. 

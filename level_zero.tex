...
\subsection{Affine Tubes}
  ...
  For each pair of vertices $i,j\in Q$, let $M_{[i,j]}$ denote the indecomposable representation of $Q$ which contains a one-dimensional vector space at each vertex $k$ contained weakly between $i$ and $j$ when tracing counterclockwise around $Q$ from $i$ to $j$.  In particular, we take $M_{[i,i]}$ to be the simple representation $S_i$ at vertex $i$.  

  For $1\le\ell\le p$ define indecomposable representations $\vec{M}_{1,\ell}:=M_{[s(\vec{a}_{\ell-1}),t(\vec{a}_\ell)]}$.  Similarly for $1\le\ell\le q$ define $\cev{M}_{1,\ell}:=M_{[t(\scev{a}_\ell),s(\scev{a}_{\ell+1})]}$.  Note that when $t(\vec{a}_\ell)$ (resp. $t(\cev{a}_\ell)$) is not a sink in $Q$ the representation $\vec{M}_{1,\ell}$ (resp. $\cev{M}_{1,\ell}$) will be simple and otherwise $\vec{M}_{1,\ell}$ (resp. $\cev{M}_{1,\ell}$) will be a string module with socle $S_{t(\vec{a}_\ell)}$ (resp. $S_{t(\scev{a}_\ell)}$) and top $S_{s(\vec{a}_{\ell-1})}$ (resp. $S_{s(\scev{a}_{\ell+1})}$).  From this we see that the collections $\{\vec{M}_{1,\ell}:1\le\ell\le p\}$ and $\{\cev{M}_{1,\ell}:1\le\ell\le q\}$ give families of pairwise orthogonal bricks.

...
\[
  \mathcal{X}_{\widetilde B} :=\{ x_{i;t}: i \in \{1,\dotsc,m\}, t \in\TT\} \subset \cF
\]
...
\begin{equation}
  (G_t^B)^T = (C_t^{-B^T})^{-1}.
\end{equation}
Note that this is not the the standard definition, but the above characterization, due to \cite[Theorem 1.2]{NZ12}, is what we will use in Section \ref{sec:exchange}; the equivalence of the two is guaranteed by sign coherence [GHKK??].
...
Moreover we will also assume that $B$ is not of finite type.

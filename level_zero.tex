...
\subsection{Affine Tubes}
  ...
  \begin{center}
    ...
    \begin{tikzpicture}
      \node at (-2,0) {$Q=$};
      \draw[fill=black] (0,0) circle (1.5pt);
      \node[right] at (0,0) {$\scriptstyle t(\scev{a}_1)^+$};
      ...
      \draw[fill=black] (0.5625,-3.1875) circle (0.75pt);
      \draw[fill=black] (0.75,-3) circle (1.5pt);
      \node[right] at (0.75,-3) {$\scriptstyle t(\scev{a}_1)_-$};
      \draw[<-] (0.75,-2.4) -- (0.75,-2.85);
      \node at (0.575,-2.675) {$\scriptstyle \scev{a}_1$};
      \draw[fill=black] (0.75,-2.25) circle (1.5pt);
      \node[right] at (0.75,-2.25) {$\scriptstyle t(\scev{a}_1)$};
      \draw[->] (0.75,-1.65) -- (0.75,-2.1);
      \draw[fill=black] (0.75,-1.5) circle (1.5pt);
      \node[right] at (0.75,-1.5) {$\scriptstyle t(\scev{a}_1)_+$};
      \draw[fill=black] (0.75,-1.3125) circle (0.75pt);
      \draw[fill=black] (0.75,-1.125) circle (0.75pt);
      \draw[fill=black] (0.75,-0.9375) circle (0.75pt);
      \draw[fill=black] (0.75,-0.75) circle (1.5pt);
  ...
  For $\ell\in\ZZ$ define indecomposable representations $\vec{M}_{1,\ell}:=M_{[s(\vec{a}_{\ell-1}),t(\vec{a}_\ell)]}$ and $\cev{M}_{1,\ell}:=M_{[t(\scev{a}_\ell),s(\scev{a}_{\ell-1})]}$.  Note that when $t(\vec{a}_\ell)$ (resp. $t(\cev{a}_\ell)$) is not a sink in $Q$ the representation $\vec{M}_{1,\ell}$ (resp. $\cev{M}_{1,\ell}$) will be simple and otherwise $\vec{M}_{1,\ell}$ (resp. $\cev{M}_{1,\ell}$) will be a string module with socle $S_{t(\vec{a}_\ell)}$ (resp. $S_{t(\scev{a}_\ell)}$) and top $S_{s(\vec{a}_{\ell-1})}$ (resp. $S_{s(\scev{a}_{\ell-1})}$).  From this we see that the collections $\{\vec{M}_{1,\ell}:1\le\ell\le p\}$ and $\{\cev{M}_{1,\ell}:1\le\ell\le q\}$ give families of pairwise orthogonal bricks, i.e. there are no nonzero morphisms between non-isomorphic representations and each of these representations has trivial endomorphism ring.
  ...
  \begin{align}
    ...
    \cev{\lambda}_{1,\ell}&=\omega_{t(\scev{a}_\ell)}-\omega_{s(\scev{a}_\ell)}.
  \end{align}
  ...
  \begin{theorem}
    ...
    \begin{enumerate}
      ...
      \item For $1\le k\le q-1$ we have
      \begin{align}
        &\Delta_{-\scev{\lambda}_{k,\ell}}(g)=\sum_{J\subset I}\left(\prod_{i\in J} t_i\prod_{i\in I\setminus J} u_i\left(u_{t(\scev{a}_\ell)_+}+\sum_{m=t(\scev{a}_\ell)_+}^{\left(t(\scev{a}_\ell)^+\right)_-}u_{m_+}\prod_{i=t(\scev{a}_\ell)_+}^m t_i+\prod_{i=t(\scev{a}_\ell)_+}^{t(\scev{a}_\ell)^+}t_i\right)^\delta\times\right.\\
  ...
  \begin{theorem}\mbox{}
    \begin{enumerate}
      ...
      \item For $1\le k\le q-1$ we have
      \begin{align}
        &x_{\scev{M}_{k,\ell}}=x^{\sum_{i\in O}\omega_i-\sum_{i\in I}\omega_i}\sum_{J\subset I}\prod_{i\in J}x_{i_-}^{-1}y_ix_{i_+}^{-1}\times\\
        \nonumber&\times\left(1+\sum_{m=t(\scev{a}_\ell)_+}^{\left(t(\scev{a}_\ell)^+\right)_-}x_{t(\scev{a}_\ell)}x_{t(\scev{a}_\ell)_+}x_m^{-1}x_{m_+}^{-1}\prod_{i=t(\scev{a}_\ell)_+}^m y_i+x_{t(\scev{a}_\ell)}x_{t(\scev{a}_\ell)_+}x_{t(\scev{a}_\ell)^+}^{-1}x_{\left(t(\scev{a}_\ell)^+\right)_+}\prod_{i=t(\scev{a}_\ell)_+}^{t(\scev{a}_\ell)^+}y_i\right)^\delta\times\\
        \nonumber&\times\sum_{P\subset\bar{J}}\prod_{j\in P}\left(x_{j^-}x_{(j^-)_+}x_j^{-2}x_{(j^+)_-}x_{j^+}\prod_{i\in(j^-,j^+)}y_i\right)\prod_{j\in I^-(P)}\left(1+\sum_{m\in(j^-,j)}x_{m_-}^{-1}x_m^{-1}x_{j_-}x_j\prod_{i=m}^{j_-}y_i\right)\times\\
        \nonumber&\hspace{0.5in}\times\prod_{j\in I^+(P)}\left(1+\sum_{m\in(j,j^+)}x_jx_{j_+}x_m^{-1}x_{m_+}^{-1}\prod_{i=j_+}^m y_i\right)
      \end{align}
      where:

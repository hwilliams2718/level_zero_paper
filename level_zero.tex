...
\subsection{Affine Tubes}
  Let $Q$ be an acyclic affine quiver of type $A$ with vertices labeled $\{0,1,\ldots,n\}$ counter-clockwise around $Q$.  It may become useful to allow arbitrary integer labeling of the vertices taken modulo $n+1$.  Suppose $Q$ has $p$ clockwise pointing arrows $\{\vec{a}_1,\ldots,\vec{a}_p\}$ labeled counterclockwise around $Q$ and $q$ counterclockwise pointing arrows $\{\cev{a}_1,\ldots,\cev{a}_q\}$ labeled clockwise around $Q$. \sayDR{This seemingly strange choice of labeling will make extensions go in the right directions later and thus keep the arrows in the AR quiver pointing from left to right.  The phrase ``against the flow'' might be appropriate to include somewhere.}  Write $i_1,\ldots,i_r$ for the sinks and sources of $Q$ read counterclockwise around the quiver and note that $r$ must be even.  We will extend this notation to include $i_t$ for $t\in\ZZ$ where $i_t=i_{t'}$ when $t\equiv t'$ modulo $r$.  After possibly relabeling, we assume $i_1=0$ is a source in $Q$ with $s(\cev{a}_q)=s(\vec{a}_p)=i_1$.  Thus $Q$ takes the following form:
  \begin{center}
    \begin{tikzpicture}
      \node at (-1,1) {$Q=$};
      \draw[fill=black] (0,0) circle (1.5pt);
      \node[below left] at (0,0) {$\scriptstyle i_1=0$};
      \draw[->] (0.1,-0.1) -- (0.65,-0.65);
      \node[below right] at (0.25,0) {$\scriptstyle \scev{a}_q$};
      \draw[fill=black] (0.75,-0.75) circle (1.5pt);
      \node[below left] at (0.75,-0.75) {$\scriptstyle  1$};
      ...
      \node[below] at (2.25,-0.75) {$\scriptstyle  i_2$};
      ...
      \draw[fill=black] (3,0.75) circle (1.5pt);
      \draw[<-] (2.4,0.75) -- (2.85,0.75);
      \node at (2.675,0.575) {$\scriptstyle \scev{a}_1$};
      \draw[fill=black] (2.25,0.75) circle (1.5pt);
      \node[above] at (2.25,0.75) {$\scriptstyle  i_r$};
      \draw[->] (1.65,0.75) -- (2.1,0.75);
      \draw[fill=black] (1.5,0.75) circle (1.5pt);
      \draw[fill=black] (1.3125,0.75) circle (0.75pt);
      \draw[fill=black] (1.125,0.75) circle (0.75pt);
      \draw[fill=black] (0.9375,0.75) circle (0.75pt);
      \draw[fill=black] (0.75,0.75) circle (1.5pt);
      \node[above left] at (0.75,0.75) {$\scriptstyle  n$};
      \draw[->] (0.1,0.1) -- (0.65,0.65);
      \node[above right] at (0.25,0) {$\scriptstyle \vec{a}_p$};
    \end{tikzpicture}
  \end{center}
  For a vertex $i$ in $Q$, write $i^+$ for the next source or sink of $Q$ counterclockwise from $i$.  Define $i^-$ similarly by looking clockwise from $i$.  Note that when $i$ is a source (resp. a sink) both $i^-$ and $i^+$ will be sinks (resp. sources).

  For each pair of vertices $i,j\in Q$, let $M_{[i,j]}$ denote the indecomposable representation of $Q$ which contains a one-dimensional vector space at each vertex $k$ contained weakly between $i$ and $j$ when tracing counterclockwise around $Q$ from $i$ to $j$.  We take $M_{[i,i]}$ to be the simple representation $S_i$ at vertex $i$.  Define full, extension-closed additive subcategories $\overrightarrow{\rep}Q$ and $\overleftarrow{\rep}Q$ of $\rep Q$ (called \emph{clockwise} and \emph{counterclockwise subcategories} respectively) as follows:
  \begin{itemize}
    ...
    \item $\overleftarrow{\rep}Q$ is generated by the representations $M_{[i_{2t},i_{2t+1}]}$ for $1\le t\le r/2$ together with the representations $S_{j+i_{2t-1}}$ for $1\le t\le r/2$ and $1\le j\le i_{2t}-i_{2t-1}-1$.
  \end{itemize}
  Notice that $\overrightarrow{\rep}Q$ is generated by exactly $p$ indecomposable representations while $\overleftarrow{\rep}Q$ is generated by exactly $q$ indecomposable representations.  Indeed, the generators of $\overrightarrow{\rep}Q$ and $\overleftarrow{\rep}Q$ can conveniently be labeled by the clockwise and counterclockwise arrows respectively.  More precisely, we will label their dimension vectors for $1\le\ell\le p$ as follows:
  \begin{equation}
    \vec{\beta}_{1,\ell}=\begin{cases}\alpha_i & \text{if $t(\vec{a}_\ell)=i$ is not a sink in $Q$}\\\alpha_{i_{s-1}}+\cdots+\alpha_{i_s} & \text{if $t(\vec{a}_\ell)=i_s$ is a sink in $Q$}\end{cases}
  \end{equation}
  and for $1\le\ell\le q$ as follows:
  \begin{equation}
    \cev{\beta}_{1,\ell}=\begin{cases}\alpha_i & \text{if $t(\cev{a}_\ell)=i$ is not a sink in $Q$}\\\alpha_{i_s}+\cdots+\alpha_{i_{s+1}} & \text{if $t(\cev{a}_\ell)=i_s$ is a sink in $Q$}\end{cases}
  \end{equation}
  This also gives a labeling of the generators of $\overrightarrow{\rep}Q$ and $\overleftarrow{\rep}Q$ respectively as $\vec{M}_{1,\ell}$ for $1\le\ell\le p$ and $\cev{M}_{1,\ell}$ for $1\le\ell\le q$.  The coindices of these representations can be described for $1\le\ell\le p$ as follows:
  \begin{equation}
  ...
  It will be convenient to allow $\ell\in\ZZ$ with identifications $\vec{M}_{1,\ell}=\vec{M}_{1,\ell+p}$ and $\cev{M}_{1,\ell}=\cev{M}_{1,\ell+q}$.  Then we define representations $\vec{M}_{k,\ell}$ (resp. $\cev{M}_{k,\ell}$) for $k\ge0$ and $\ell\in\ZZ$ inductively via the short exact sequence
  ...
  It follows from the Proposition that the dimension vectors of the exceptional regular representations are given by
  \begin{equation}
    \vec\beta_{k,\ell}=\sum\limits_{m=1}^k\vec\beta_{1,\ell+m-1}\quad\text{for $1\le k\le p-1$ and $1\le\ell\le p$}
  \end{equation}
  and
  \begin{equation}
    \cev\beta_{k,\ell}=\sum\limits_{m=1}^k\cev\beta_{1,\ell+m-1}\quad\text{for $1\le k\le q-1$ and $1\le\ell\le q$.}
  \end{equation}
  ...
  \begin{lemma}\mbox{}
    \begin{enumerate}
      ...
      \[\vec{\lambda}_{k,\ell}=\sum_{i\in I}\omega_i-\sum_{i\in\bar{I}}\omega_i\]
      where $I$ denotes the sinks in the support quiver $Q_{k,\ell}$ of $\vec{M}_{k,\ell}$ and $\bar{I}$ denotes the set of internal sources of $\vec{Q}_{k,\ell}$ together with $s(\vec{a}_{k+\ell-1})$.
      \item For $1\le k\le q-1$ and $1\le\ell\le q$ we have
      \[\cev{\lambda}_{k,\ell}=\sum_{i\in I}\omega_i-\sum_{i\in\bar{I}}\omega_i\]
      where $I$ denotes the sinks in the support quiver $Q_{k,\ell}$ of $\cev{M}_{k,\ell}$ and $\bar{I}$ denotes the set of internal sources of $\cev{Q}_{k,\ell}$ together with $s(\cev{a}_{k+\ell-1})$.
    \end{enumerate}
  \end{lemma}

  Let $\sigma\in\Sigma_{n+1}$ denote a permutation of the vertices of $Q$ such that $\sigma_0$ is a sink in $Q$ and $\sigma_i$ is a sink in $\mu_{i-1}\cdots\mu_0 Q$ for $1\le i\le n$.  Let $c=s_{\sigma_0}\cdots s_{\sigma_n}$ denote the Coxeter element adapted to $Q$.
  \begin{theorem}
    ...
    \begin{enumerate}
      \item For $1\le k\le p-1$ and $1\le\ell\le p$ we have
      \begin{align}
        &\Delta_{\vec{\lambda}_{k,\ell}}(g)=\sum_{J\subset I}\left(\prod_{i\in J} t_i\prod_{i\in I\setminus J} u_i^{-1}\left(\prod_{i:t(\vec{a}_\ell)^-\le i<t(\vec{a}_\ell)}t_i+\sum_{m:t(\vec{a}_\ell)^-<m\le t(\vec{a}_\ell)}u_{m-1}^{-1}\prod_{i=m}^{t(\vec{a}_\ell)-1} t_i\right)^\delta\times\right.\\
        \nonumber&\times\left.\sum_{O\subset\bar{J}}\prod_{\substack{i:j^-<i<j^+\\j\in O}}t_i\prod_{i\in\bar{I}\setminus O}u_i\left(\prod_{j\in I^-(O)}\sum_{m:j^-<m\le j}u_{m-1}^{-1}\prod_{i=m}^{j-1} t_i\right)\left(\prod_{j\in I^+(O)}\sum_{m:j\le m<j^+}u_{m+1}^{-1}\prod_{i=j+1}^{m} t_i\right)\right)
      \end{align}
      where:
      \begin{itemize}
        \item $I$ denotes the sinks in the support quiver $\vec{Q}_{k,\ell}$ of $\vec{M}_{k,\ell}$ read counterclockwise around $Q$,
        \item $\bar{I}$ denotes the set of internal sources of $\vec{Q}_{k,\ell}$ together with $s(\vec{a}_{k+\ell-1})$,
        \item for $J\subset I$, $\bar{J}$ denotes the set of sources $j$ in the support quiver $\vec{Q}_{k,\ell}$ for which $j^-,j^+\in J$,
        \item for $O\subset\bar{J}$, $I^\pm(O)=\{j\in J:j^\pm\in\bar{I}\setminus O\}$,
        \item $\delta=\begin{cases}1 & \text{if $t(\vec{a}_\ell)$ is a sink in $Q$ and $t(\vec{a}_\ell)\in J$;}\\0 & \text{otherwise.}\end{cases}$
      \end{itemize}
      \item For $1\le k\le q-1$ and $1\le\ell\le q$  we have
      \begin{align}
        &\Delta_{\scev{\lambda}_{k,\ell}}(g)=\sum_{J\subset I}\left(\prod_{i\in J} t_i\prod_{i\in I\setminus J} u_i^{-1}\left(\prod_{i:t(\scev{a}_\ell)<i\le t(\scev{a}_\ell)^+}t_i+\sum_{m:t(\scev{a}_\ell)\le m<t(\scev{a}_\ell)^+}u_{m+1}^{-1}\prod_{i=t(\scev{a}_\ell)+1}^m t_i\right)^\delta\times\right.\\
        \nonumber&\times\left.\sum_{O\subset\bar{J}}\prod_{\substack{i:j^-<i<j^+\\j\in O}}t_i\prod_{i\in\bar{I}\setminus O}u_i\left(\prod_{j\in I^-(O)}\sum_{m:j^-<m\le j}u_{m-1}^{-1}\prod_{i=m}^{j-1} t_i\right)\left(\prod_{j\in I^+(O)}\sum_{m:j\le m<j^+}u_{m+1}^{-1}\prod_{i=j+1}^{m} t_i\right)\right)
      \end{align}
      where:
      \begin{itemize}
        \item $I$ denotes the sinks in the support quiver $\cev{Q}_{k,\ell}$ of $\cev{M}_{k,\ell}$ read clockwise around $Q$,
        \item $\bar{I}$ denotes the set of internal sources of $\cev{Q}_{k,\ell}$ together with $s(\cev{a}_{k+\ell-1})$,
        \item for $J\subset I$, $\bar{J}$ denotes the set of sources $j$ in the support quiver $\cev{Q}_{k,\ell}$ for which $j^-,j^+\in J$,
        \item for $O\subset\bar{J}$, $I^\pm(O)=\{j\in J:j^-\in\bar{I}\setminus O\}$,
        \item $\delta=\begin{cases}1 & \text{if $t(\cev{a}_\ell)$ is a sink in $Q$ and $t(\cev{a}_\ell)\in J$;}\\0 & \text{otherwise.}\end{cases}$
      \end{itemize}
    \end{enumerate}
  \end{theorem}
  \begin{proof}
    The calculation breaks into several independent type $A$ weight space computations coming from consecutive strings of clockwise or consecutive counterclockwise arrows in $Q$.  The set $I$ records the positive components of the weight $\vec{\lambda}_{k,\ell}$ which all correspond to sinks in $Q_{k,\ell}$.  In each case such a string of equi-oriented arrows is only considered if a step down in weight was performed at the sink $i\in I$ of that path, i.e. if the action of $x_i(t_i)$ replaced the current weight $\omega$ by $\omega-\alpha_i$ in which case the $x_{\bar{i}}(u_i)$ will contribute nothing later.  This accounts for the sum over all subsets $J$ as well as the first product of $t_i$'s and the first product of the $u_i^{-1}$'s (steps not taken when acting by $x_i(t_i)$ and thus no change in weight by $x_{\bar{i}}(u_i)$).

    Next, one has to consider each of these strings separately (mostly).  The set $\bar{I}$ records the negative components of the weight $\vec{\lambda}_{k,\ell}$ and thus if no change in weight is made by the action of $x_{\bar{i}}(u_i)$ for $i\in\bar{I}$ there will be a factor of $u_i$.  The set $O$ records those sources $j$ where every $x_i(t_i)$ along \emph{both} equi-oriented paths beginning at $j$ replaced the current weight $\omega$ by $\omega-\alpha_i$ in which case each of the $x_{\bar{i}}(u_i)$ along these paths will contribute nothing later.  This accounts for the products over elements of $O$ and $\bar{I}\setminus O$.  

    Finally, along each equi-oriented path one can choose to act by some of the $x_i(t_i)$ in a contiguous string beginning at the sink, this accounts for the two products of sums, one for clockwise paths and one for counterclockwise paths.  The $\delta$ is a slight annoyance present in the weight $\vec{\lambda}_{k,\ell}$, there is one equi-oriented string whose sink appears in $\vec{\lambda}_{k,\ell}$ with positive coefficient but whose source does not appear so it was hard to find uniform notation to capture that string with the others.
  \end{proof}

...
\begin{equation}\label{eq:matrix mutation}
  ...
  \end{cases}
\end{equation}
\saySS{I changed $B_{ij}$ into $b_{ij}$ for uniformity with Cartan matrix notation}\sayDR{Are tildes necessary here?}
...
\begin{definition}[\cite{FZ02}]
  ...
  \saySS{Notation here will become cluttered quite soon. How do you feel about changing $\cA_{\widetilde B}$ to $\cA(\widetilde B)$ and $\mathcal{X}_{\widetilde B}$ to $\mathcal{X}(\widetilde B)$?}
\end{definition}

For each $t \in\TT$ and $1 \leq i \leq n$ there is an auxiliary pair of integer vectors depending only on $B$, the \newword{$\bfc$-vector} $\gamma_{i;t} \in \ZZ^n$ and the \newword{$\bfg$-vector} $\lambda_{i;t} \in \ZZ^n$. 
...
For $t\in\TT$, compute $\widetilde B_\bullet^t$ by matrix mutations \eqref{eq:matrix mutation} from $\widetilde B_\bullet^{t_0}=\widetilde B_\bullet$, then $C_t^B$ is defined to be the bottom $n \times n$ block of $\widetilde B_\bullet^t$. 

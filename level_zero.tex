...
\begin{equation}\label{eq:matrix mutation}
  ...
  \end{cases}
\end{equation}
\saySS{I changed $B_{ij}$ into $b_{ij}$ for two reasons: 1) uniformity with Cartan matrix notation; 2) avoid confusion with $B_{\bullet \bullet}$ if we can't find a better notation for this.}
...
The goal of this section is to realize geometrically the ``doubled pricipal coefficients'' cluster algebra $\cA_{\widetilde B_{\bullet\bullet}}$, given by the $3n\times n$ extensions $\widetilde B_{\bullet\bullet}$ of $B$ by two identity matrices, in terms of the derived Kac-Moody group $G$ associated to $A$. 
\saySS{here I am temporarely setting up the notation $\widetilde B_{\bullet\bullet}$. We may want to change this to something else. Giovanni suggests to put the bullets one ontop of the other since we stack matrices.}
...
\begin{theorem}
  ...
  Then there is an isomorphism $\cA_{\widetilde B_{\bullet\bullet}} \cong \kk[G^{c,c^{-1}}]$ that identifies the initial cluster variables with the following restrictions of monomials in generalized minors
  \begin{equation}
...
\end{theorem}
\begin{proof}
  \saySS{This needs to be completely rewritten: mention Harold's and explain change of coefficients}
...
In particular the initial cluster variables of $\cA_{\widetilde B_{\bullet\bullet}}$ evaluates to $x_i(g)=h^{\omega_i}$.
...
In this section we prove that when $B$ is any acyclic skew-symmetrizable matrix, the identification $\cA_{\widetilde B_{\bullet\bullet}} \cong \kk[G^{c,c^{-1}}]$ of \Cref{thm:coordring} realizes preprojective and postinjective cluster variables as restrictions of some highest- and lowest-weight minors, respectively. 
...
\subsection{The bipartite belt of $\cA_{\widetilde B_{\bullet\bullet}}$}
...
With the preceding results in hand, we are ready to describe the cluster variables of $\cA_{\widetilde B_{\bullet\bullet}}$ attached to the acyclc belt.

\saySS{Is this a theorem or a proposition?}
\begin{theorem}
  \label{thm:vars_and_rels_in_bipartite_belt}
  The preprojective and postjective cluster variables of $\cA_{\widetilde B_{\bullet\bullet}}$ are parametrized by the sets of $\bfg$-vectors
  \[
...
\begin{proof}
  ...
  The relations are then an obvious consequence of \Cref{lemma:c-vectors} once we observe that mutations happen along a source/sink adapted sequence so that, at $t_m$, the $\mod{m}$-th colum of $B_m$ contains only non-negative entries and the $\mod{m+1}$-th column only non-positive entries. 
\end{proof}
...
%  \begin{theorem}
%    For $x_i=\Delta_{\omega_i}$ and $y_i$ as above, we have $x_{\vec{M}_{k,\ell}}=\Delta_{-\vec{\lambda}_{k,\ell}}$ and $x_{\scev{M}_{k,\ell}}=\Delta_{-\scev{\lambda}_{k,\ell}}$ on $L^{c,c^{-1}}$.
%  \end{theorem}
%  \begin{proof}\mbox{}
%    \begin{itemize}
...
%      \item
%      \[\left(x_{t(\scev{a}_\ell)}x_{t(\scev{a}_\ell)_+}x_{t(\scev{a}_\ell)^+}^{-1}x_{\left(t(\scev{a}_\ell)^+\right)_+}\prod_{i=t(\scev{a}_\ell)_+}^{t(\scev{a}_\ell)^+}y_i\right)(g)=u_{t(\scev{a}_\ell)_+}^{-1}\prod_{i=t(\scev{a}_\ell)_+}^{t(\scev{a}_\ell)^+}t_i\]
%    \end{itemize}

...
\subsection{Affine Tubes}
  ...
  \begin{theorem}
    ...
    \begin{enumerate}
      \item For $1\le k\le p-1$ we have
      \begin{align}
        &\Delta_{-\vec{\lambda}_{k,\ell}}(g)=\sum_{J\subset I}\left(\prod_{i\in J} t_i\prod_{i\in I\setminus J} u_i\left(\prod_{i=t(\vec{a}_\ell)^-}^{t(\vec{a}_\ell)_-}t_i+\sum_{m=\left(t(\vec{a}_\ell)^-\right)_+}^{t(\vec{a}_\ell)_-}u_{m_-}\prod_{i=m}^{t(\vec{a}_\ell)_-} t_i+u_{t(\vec{a}_\ell)_-}\right)^\delta\times\right.\\
      ...
      \item For $1\le k\le q-1$ we have
      \begin{align}
        &\Delta_{-\scev{\lambda}_{k,\ell}}(g)=\sum_{J\subset I}\left(\prod_{i\in J} t_i\prod_{i\in I\setminus J} u_i\left(\prod_{i=t(\scev{a}_\ell)_+}^{t(\scev{a}_\ell)^+}t_i+\sum_{m=t(\scev{a}_\ell)_+}^{\left(t(\scev{a}_\ell)^+\right)_-}u_{m_+}\prod_{i=t(\scev{a}_\ell)_+}^m t_i+u_{t(\scev{a}_\ell)_+}\right)^\delta\times\right.\\
  ...
  \begin{theorem}\mbox{}
    \begin{enumerate}
      \item For $1\le k\le p-1$ we have
      \begin{align}
        &x_{\vec{M}_{k,\ell}}=x^{\sum_{i\in O}\omega_i-\sum_{i\in I}\omega_i}\sum_{J\subset I}\prod_{i\in J}x_{i_-}^{-1}y_ix_{i_+}^{-1}\left(\sum_{m=t(\vec{a}_\ell)^-}^{t(\vec{a}_\ell)_-}x_m^{-1}x_{t(\vec{a}_\ell)}\prod_{i=m}^{t(\vec{a}_\ell)_-}y_i\right)^\delta\times\\
        \nonumber&\times\sum_{P\subset\bar{J}}\prod_{j\in P}\left(x_{j^-}x_j^{-2}x_{j^+}\prod_{i\in(j^-,j^+)}y_i\right)\left(\prod_{j\in I^-(P)}\sum_{m\in(j^-,j)}x_{m_-}^{-1}x_j\prod_{i=m}^{j_-}y_i\right)\left(\prod_{j\in I^+(P)}\sum_{m\in(j,j^+)}x_jx_{m_+}^{-1}\prod_{i=j_+}^m y_i\right)
      \end{align}
      where:
      \begin{itemize}
        \item $I$ denotes the sinks in the support quiver $\vec{Q}_{k,\ell}$ of $\vec{M}_{k,\ell}$ read counterclockwise around $Q$,
      ...
      \item For $1\le k\le q-1$ we have
      \begin{align}
        &x_{\scev{M}_{k,\ell}}=x^{\sum_{i\in O}\omega_i-\sum_{i\in I}\omega_i}\sum_{J\subset I}\prod_{i\in J}x_{i_-}^{-1}y_ix_{i_+}^{-1}\left(\sum_{m=t(\scev{a}_\ell)_+}^{t(\scev{a}_\ell)^+}x_m^{-1}x_{t(\scev{a}_\ell)}\prod_{i=m}^{t(\scev{a}_\ell)_-}y_i\right)^\delta\times\\
        \nonumber&\times\sum_{P\subset\bar{J}}\prod_{j\in P}\left(x_{j^-}x_j^{-2}x_{j^+}\prod_{i\in(j^-,j^+)}y_i\right)\left(\prod_{j\in I^-(P)}\sum_{m\in(j^-,j)}x_{m_-}^{-1}x_j\prod_{i=m}^{j_-}y_i\right)\left(\prod_{j\in I^+(P)}\sum_{m\in(j,j^+)}x_jx_{m_+}^{-1}\prod_{i=j_+}^m y_i\right)
      \end{align}
      where:
      \begin{itemize}

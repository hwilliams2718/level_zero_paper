...
\end{proposition}
\begin{proof}
  In the ``opposite'' double Bruhat cell $G^{c^{-1},c}$ \cref{prop:projective_minors_identity} says that 
  \[
    \Delta_{c^{-k}\omega_i}
    \Delta_{c^{-k-1}\omega_i}
    =
    \prod_{j<i}\Delta_{c^{-k}\omega_j}^{-a_{ji}}
    \prod_{j>i}\Delta_{c^{-k-1}\omega_j}^{-a_{ji}}
    +
    ...
    \right)^{[c^{-k}\beta_i^-:\alpha_j]}
  \]
  and recalling that $\Delta_\lambda^\mu(x)\Delta_{-\mu}^{-\lambda}(x^\iota)$ we get
  \[
    \Delta_{-c^{-k}\omega_i}
    \Delta_{-c^{-k-1}\omega_i}
    =
    ...
    \right)^{[c^{-k}\beta_i^-:\alpha_j]}
  \]
  and the results follows from \cref{lemma:coefficients_identities}.
\end{proof}
...
\begin{lemma}
  ...
  \[
    ...
    \prod_{i<j}\big(\Delta_{\omega_i}^{c\omega_i}\big)^{a_{ij}}
    =
    \Delta_{-c^{-1}\omega_j}^{-\omega_j}
    \prod_{i>j}\big(\Delta_{-c^{-1}\omega_i}^{-\omega_i}\big)^{a_{ij}}
  \]
  \[
    \Delta_{c\omega_j}^{\omega_j}
    \prod_{i<j}\big(\Delta_{c\omega_i}^{\omega_i}\big)^{a_{ij}}
    =
    \Delta_{-\omega_j}^{-c^{-1}\omega_j}
    \prod_{i>j}\big(\Delta_{-\omega_i}^{-c^{-1}\omega_i}\big)^{a_{ij}}
  \]
\end{lemma}

...
To define the (geometric type) cluster algebra $\cA_{\tilde B}$ associated to an $m \times n$ exchange matrix $\tilde B$, we begin with an infinite $n$-ary tree $\TT$ with edges labeled by $\{1,\dotsc,n\}$ so that the edges incident to any given vertex have distinct labels.
Fix a root vertex $t_0 \in\TT$.  Assign an exchange matrix $\tilde B^t$ to each $t \in\TT$ so that:
\begin{itemize}
  \item $\tilde B^{t_0} = \tilde B$;
  \item if $t, t' \in\TT$ are joined by an edge labeled $k$, then $\tilde B^t$ and $\tilde B^{t'}$ are related by mutation at $k$.
\end{itemize}
...
\]
\saySS{should we add ``associated to $\tilde B$'' ore something?}\sayDR{maybe add ``of $\cA_{\tilde B}$''?}
...
\saySS{I am temporarely using the bullet notation, we should fix this later}\sayDR{I added $\widetilde{}$s so that it's clear they are exchange matrices, again we can change later}
...
The terminology is justified by the skew-symmetric case: a skew-symmetric matrix $B$ can be recovered from the quiver $Q := Q_B$ with vertices indexed by the columns of $B$ and $[-B_{ij}]_+$ arrows to vertex $i$ from vertex $j$; the skew-symmetric matrix is acyclic exactly when this quiver has no oriented cycles.
